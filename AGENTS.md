...
- Before a page or lab PR: `python3 scripts/shed_check.py` (runs cook_guard, page56 lab, unit lab, Ensure-TweetCard, precache check, headers check; no new locators, no secrets, no solve claims).
- After a new chamber: add it to `public/sw.js` `PRECACHE`, bump `CACHE`, run `python3 scripts/precache_check.py`.
