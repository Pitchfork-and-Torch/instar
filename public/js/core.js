...
(function () {
  ...
  function resume() {
    const next = nextId();
    const href = PATHS[next] || PATHS.hello || "/";
    if (complete()) {
      return { show: false, href: "/manual/", label: "Continue the molt" };
    }
    return {
      show: depth() > 1 && !!PATHS[next] && next !== "hello",
      href: href,
      label: "Continue the molt",
    };
  }
  function norm(s) {
    return String(s || "")
  ...
  function mountContinue() {
    const el = document.getElementById("continue-molt");
  ...
  function bindGate(opts) {
    ...
    if (status) {
      ...
      if (status.id && described.indexOf(status.id) < 0) described.push(status.id);
