...
#   - cook_guard, page56 lab, unit lab, tweet card, precache, headers, listing, page-lab contract, door, cook map, manifest, EN-only, canonical, llms, icon, fresh, hits, or skip+main no longer hold
...
def main() -> int:
    ...
    fails += run_tool(
        [sys.executable, str(SCRIPTS / "llms_check.py")], "LLMS OK"
    )
    fails += run_tool(
        [sys.executable, str(SCRIPTS / "icon_check.py")], "ICON OK"
    )
    fails += run_tool(
        [sys.executable, str(SCRIPTS / "fresh_check.py")], "FRESH OK"
    )
    fails += run_tool(
        [sys.executable, str(SCRIPTS / "hits_check.py")], "HITS OK"
    )
    fails += run_tool(
        [sys.executable, str(SCRIPTS / "skip_check.py")], "SKIP OK"
    )
    fails += scan_tree()
    if fails:
        ...
        return 1
    print("SHED CHECK OK")
    print("public school holds: cook guard, page56 lab, unit lab, tweet card, precache, headers, listing, page-lab contract, door, cook map, manifest, EN, canon, llms, icon, fresh, hits, skip,")
