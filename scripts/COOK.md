...
Desk Playwright (`scripts/verify_lab.mjs`) is not in CI. Chromium first, then a local Playwright Chromium cache or Chrome. Do not use Edge. It covers skip+main on molt pages, Continue the molt from hello storage, and gate aria-live. Caesar NaN-key, LSB miss (green bit 3 vs red LSB), and resume / complete molt live in `unit_lab.js` against `core.js`.
