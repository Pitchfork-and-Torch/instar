...
def scan_public() -> list[str]:
    ...
    for path in pages:
        ...
        low = text.lower()
        skip_at = low.find('class="skip"')
        header_at = low.find("<header")
        main_at = low.find('id="main"')
        if header_at != -1 and skip_at > header_at:
            fails.append(rel + " skip is after header")
        if main_at != -1 and skip_at > main_at:
            fails.append(rel + " skip is after main")
    return fails
